...
check_template_drift() {
...

    {
        ...
        echo "3. Add one row per commit (or per group, for a multi-commit rewrite) to"
        echo "   [docs/template-drift-log.md](${REPO_URL}/docs/template-drift-log.md) —"
        echo "   the running ledger of every template commit reviewed, port-or-skip and"
        echo "   why. A range with a real behavioral change also gets its own"
        echo "   [ADR](${REPO_URL}/docs/decisions/TEMPLATE.md) with the full reasoning,"
        echo "   linked from the log row rather than duplicated there. See"
        ...
        echo "4. Once every commit in the range has a log row (ported, or deliberately"
