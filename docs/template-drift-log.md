...
| [`d95b282b`](https://github.com/ublue-os/image-template/commit/d95b282bd679b897ecb334eec5a589a5e2ae701c) | 2026-08-27 | `build.yml`, `build-disk.yml` | `runs-on` bumped `ubuntu-24.04` → `ubuntu-26.04` (`-arm` variant too), working around podman-upgrade breakage on the 24.04 runner image. | **Ported** — real runner-OS change, not a Renovate-managed digest bump | #54 |
| [`7e97f6ee`](https://github.com/ublue-os/image-template/commit/7e97f6ee047f031561343040148a983049ec3f4d) → [`1dd3b656`](https://github.com/ublue-os/image-template/commit/1dd3b656c4eb99dea9fbf9bd8210a754bd94adab) | 2026-09-04 – 2026-09-05 | `Containerfile` | Template's `bazzite:stable` base pin unpinned from a digest back to the floating tag, then re-pinned to a newer digest — churn around keeping their own digest pin current. | **Skipped** — we don't follow this pattern at all; our base is pinned to a Fedora-versioned tag (`bazzite-dx:stable-44`), not a floating tag or a digest, precisely to avoid this kind of drift (ADR-0003, ADR-202608222345) | [ADR-0003](decisions/0003-pin-base-to-fedora-versioned-tag.md) |
| [`9bd722c5`](https://github.com/ublue-os/image-template/commit/9bd722c57a48d14c1c5986870d91d534b1aba09c) | 2026-09-05 | `build-disk.yml` | Bump `ublue-os/remove-unwanted-software` action past `v9`, which breaks on `ubuntu-26.04` runners; no `v10` tag was ever cut so Renovate can't see the newer commit as an update. | **Ported** — `build-disk.yml` moved to `ubuntu-26.04` in #56 but this action wasn't bumped alongside it; `build.yml` already carried the newer digest | #60 |
